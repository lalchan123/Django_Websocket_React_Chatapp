    ...
    useEffect(() => {
        const fetchInterest = async () => {
            try {
                ...
                const response = await axios.get('http://127.0.0.1:8080/api/accept-interest/', {
                    headers: {
        ...
        if (selectedUser) {
            ...
            // const ws = new WebSocket(`ws://localhost:8001/ws/chat/?token=${accessToken}&recipient=${selectedUser.id}`);
            const ws = new WebSocket(`ws://localhost:8080/ws/chat/?token=${accessToken}&recipient=${selectedUser.id}`);
